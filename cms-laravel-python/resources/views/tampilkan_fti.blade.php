...
<div class="form-group select-form">
    ...
    {{ csrf_field() }}
        <select name="select_prodi" id="select_prodi" class="select-tahun" onchange="location = this.value;">
            ...
            <option value="{{ URL::to('/programstudi/fti/1')}}" {{ isset($select_prodi) ? ($select_prodi == 1 ? 'selected' : ' ') : ' ' }}>
                Fisika
            </option>
            <option value="{{ URL::to('/programstudi/fti/2')}}" {{ isset($select_prodi) ? ($select_prodi == 2 ? 'selected' : ' ') : ' ' }}>
                Elektro
            </option>
            <option value="{{ URL::to('/programstudi/fti/3')}}" {{ isset($select_prodi) ? ($select_prodi == 3 ? 'selected' : ' ') : ' ' }}>
                Ekonometrika
            </option>
        </select>
    </form>
